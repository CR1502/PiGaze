...
class MPIIFaceGazeDataset(Dataset):
    def __init__(self, root_dir, transform=None, participants=range(15)):
        self.root_dir = root_dir
        self.transform = transform
        self.samples = []
        self.participants = participants

        for participant in self.participants:  # 15 participants from p00 to p14
            #participants p11-p14 will be used for testing the model
...
data_path = os.path.join(os.getcwd(), 'data', 'dataset',  'MPIIFaceGaze', 'Data', 'Data')
...
training_dataset = MPIIFaceGazeDataset(root_dir=data_path, transform=transform,  participants=range(11)) #p00 to p10
test_dataset = MPIIFaceGazeDataset(root_dir=data_path, transform=transform, participants=range(11, 15))  # p11 to p14

# Split dataset into train and validation sets
train_size = int(0.7 * len(training_dataset)) # you an also modify size of the training set
val_size = len(training_dataset) - train_size
train_dataset, val_dataset = torch.utils.data.random_split(training_dataset, [train_size, val_size])
...
# Training loop
num_epochs = 200  # play around with this no.
