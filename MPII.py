...
class MPIIFaceGazeDatasetH(Dataset):
    def __init__(self, root_dir, transform=None):
        self.root_dir = root_dir
        self.transform = transform
        self.samples = []
    ...
    def __getitem__(self, idx):
        ...
        #label = np.concatenate([sample['gaze_location'], sample['head_pose']])
        label = sample['gaze_location']
        return head_pose, torch.tensor(label, dtype=torch.float32)
...
datasetH = MPIIFaceGazeDatasetH(root_dir=data_path, transform=transform)

# Split dataset into train and validation sets
train_size = int(0.9 * len(dataset)) # you an also modify size of the taining set
val_size = len(dataset) - train_size
train_dataset, val_dataset = torch.utils.data.random_split(dataset, [train_size, val_size])
train_datasetH, val_datasetH = torch.utils.data.random_split(datasetH, [train_size, val_size])

batch_size = 64
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=batch_size)

train_loaderH = DataLoader(train_datasetH, batch_size=batch_size, shuffle=True)
val_loaderH = DataLoader(val_datasetH, batch_size=batch_size)
...
for epoch in range(num_epochs):
    model.train()
    train_loss = 0.0
    for batch_idx, (i1, i2) in enumerate(zip(train_loader, train_loaderH)):
        data, target = i1
        dataH, targetH = i2

        data, target = data.to(device), target.to(device)
        dataH, targetH = dataH.to(device), targetH.to(device)

        output = model(data, dataH)
...
ax.set_title('Loss and Val Loss')
ax.set_xlabel('Epoch')
ax.set_ylabel('Loss')
ax.legend()
...
def predict_gaze(model, image_path, transform, true_head_pose):
    image = Image.open(image_path).convert('RGB')
    image = transform(image).unsqueeze(0)
    with torch.no_grad():
        output = model(image, true_head_pose)
...
gaze_target = predict_gaze(trained_model, test_image_path, transform, torch.tensor([true_head_pose]))
...
image = cv2.imread(r'C:\Users\justi\Documents\NEU\Fall_2024\MLFoundations\GroupProject\Screenshot 2024-11-21 201902.png')
